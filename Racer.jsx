...
function buildCircuit() {
  ...
  // Long pit straight → Copse right → Maggotts/Becketts S-bends → Hangar straight
  // → Stowe hairpin → Vale → Club → back to pit straight
  const pts = [
    // Pit straight (right to left in X, flat in Z)
    [ 18,   2], [ 10,   2], [  2,   2],
    // Copse – fast right-hander
    [ -4,   2], [ -9,  -2], [-11,  -8],
    // Maggotts – left kink
    [-12, -14], [ -9, -18],
    // Becketts – tight right then left S
    [ -4, -21], [  2, -20], [  6, -24],
    // Chapel – right
    [  4, -28], [ -1, -31],
    // Hangar straight – long left-to-right diagonal
    [ -6, -34], [-10, -37], [-14, -36],
    // Stowe – slow right hairpin
    [-18, -32], [-20, -27], [-18, -22],
    // Vale – quick left
    [-15, -17], [-10, -15],
    // Club – sweeping right back to start
...
export default function MeetingGP() {
  ...
  const mmRef      = useRef(null);   // minimap canvas
  ...
  useEffect(()=>{
    ...
    /* Camera — initialised after circuit is built below */
    ...
    /* Lighting – Bruno Simon golden sun */
    ...
    /* Set camera start position now that circuit bounds are known */
    ...
    racersRef.current = raceNames.map((name,i)=>{
      ...
      return {
        name, hex, car, sprite,
        totalT: -i*0.018,   // staggered grid start
        lap: 1,
        // Wider base speed range — cars have genuine pace differences
        baseSpeed: BSPD * (0.78 + Math.random()*0.44),
        // Momentum — current actual speed, lerps toward target each frame
        momentum: BSPD,
        nitro: 0, nitroCD: 0,
        // Cornering penalty — cars slow more in tight sections
    ...
    /* ── Camera state — single smooth overhead tracker ── */
    ...
    const animate = ()=>{
      ...
      if (!doneRef.current) {
        ...
        racersRef.current.forEach((r,ri)=>{
          ...
          // Current track position (pre-update) — needed for curvature
          const tCur = ((r.totalT % 1) + 1) % 1;
          const posCur = circuit.getPointAt(tCur);

          // Cornering: curvature proxy from neighbouring points
          ...
          // Lap-based speed surge: cars push harder in final laps
          const lapFraction = Math.min(r.lap / TOTAL_LAPS, 1);
          const lapBoost = lapFraction * BSPD * 0.18;

          // Target speed with all factors
          ...
          // Momentum — smooth acceleration/braking
          ...
          // New track position for rendering
          const t   = ((r.totalT % 1) + 1) % 1;
          const pos = circuit.getPointAt(t);
          // Use finite difference for direction — more reliable than getTangentAt
          const tAhead = (t + 0.008) % 1;
          const ahead  = circuit.getPointAt(tAhead);
          const dx = ahead.x - pos.x;
          const dz = ahead.z - pos.z;
          // atan2(-dz, dx) aligns local +X (car nose) with travel vector (dx,0,dz)
        ...
        if (frame%5===0 && mmRef.current) {
          ...
          // Dark background — no transparent bleed
          mc.fillStyle="#0a0a0a";
        ...
        /* ── Camera: spread-based overhead — slight Z tilt avoids degenerate up-vector ── */
        let cx=0, cz=0;
        ...
        // Slight +Z offset stops camera looking perfectly straight down (degenerate up-vector)
        // This also gives a mild perspective that makes cars visually face their direction of travel
  ...
  }, [screen, raceKey]); // eslint-disable-line
  ...
  if (screen==="race") return (
    <div style={{fontFamily:'"Titillium Web",sans-serif',width:"100vw",height:"100vh",position:"relative",overflow:"hidden",background:"#050508"}}>
      ...
      {/* Three.js canvas mount — explicit z:1 so overlays always win */}
      <div ref={mountRef} style={{position:"absolute",inset:0,zIndex:1}}/>

      {/* Full overlay — z:20, pointer-events none so clicks pass through */}
      <div style={{position:"absolute",inset:0,zIndex:20,pointerEvents:"none"}}>
        ...
        {/* Right side: leaderboard — starts below lap counter */}
  ...
  if (screen==="winner") return (
    <div style={{fontFamily:'"Titillium Web",sans-serif',background:"#000",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"#fff",overflow:"hidden",position:"relative"}}>
      ...
      {/* Full bleed colour flash background */}
